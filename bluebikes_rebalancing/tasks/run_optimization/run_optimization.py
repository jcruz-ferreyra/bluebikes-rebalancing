...
def _build_model(stations_df: pd.DataFrame, network_df: pd.DataFrame, ctx: RunOptimizationContext):
    """
    Build the multi-vehicle Pyomo optimization model from prepared data.
    ...
    logger.info(f"Fleet: up to {K} vehicles of capacity {Q}, depot capacity {S}")
...
# ----------------------------------------------------------------------------
# Solution-extraction helpers (fleet-aware)
# ----------------------------------------------------------------------------


def _vehicle_used(model, k) -> bool:
    """True if vehicle k leaves the start depot to serve a station."""
    return (
        sum(
            value(model.x[i, j, k])
            for (i, j) in model.ARCS
            if i == DEPOT_START and j != DEPOT_END
        )
        > 0.5
    )


def _station_vehicle_map(model) -> dict:
    """Map each served station to the vehicle that serves it (None if unserved)."""
    mapping = {}
    for h in model.STATIONS:
        for k in model.VEHICLES:
            served = sum(value(model.x[i, j, k]) for (i, j) in model.ARCS if j == h) > 0.5
            if served:
                mapping[h] = k
                break
    return mapping
...
def _node_pickups(model, i) -> int:
    """Total bikes picked up at node i across the fleet."""
    return int(round(sum(value(model.u[i, k]) for k in model.VEHICLES)))


def _node_dropoffs(model, i) -> int:
    """Total bikes dropped off at node i across the fleet."""
    return int(round(sum(value(model.v[i, k]) for k in model.VEHICLES)))


def _vehicle_metrics(model, k) -> dict:
    """Per-vehicle distance, time components and visit count."""
    distance_m = sum(value(model.dist[i, j]) * value(model.x[i, j, k]) for (i, j) in model.ARCS)
    travel_time = sum(value(model.ttime[i, j]) * value(model.x[i, j, k]) for (i, j) in model.ARCS)
    station_time = sum(
        value(model.s) * sum(value(model.x[ia, j, k]) for (ia, j) in model.ARCS if ia == i)
        + value(model.tau) * (value(model.u[i, k]) + value(model.v[i, k]))
        for i in model.STATIONS
    )
    depot_time = value(model.tau) * (
        value(model.u[DEPOT_START, k]) + value(model.v[DEPOT_END, k])
    )
    num_stations = sum(
        1
        for h in model.STATIONS
        if sum(value(model.x[i, j, k]) for (i, j) in model.ARCS if j == h) > 0.5
    )
    bikes_moved = int(round(sum(value(model.u[i, k]) for i in model.NODES)))
    return {
...
def _order_routes_by_sequence(route_df: pd.DataFrame) -> pd.DataFrame:
    """Order route arcs by visit sequence, independently per vehicle, each
    starting from the depot."""
    ordered_frames = []
    for veh, veh_df in route_df.groupby("vehicle"):
        ordered = []
        remaining = veh_df.reset_index(drop=True)
        current = DEPOT_START
        while len(ordered) < len(veh_df):
            arc = remaining[remaining["from"] == current]
            if arc.empty:
                break
            ordered.append(arc.iloc[0])
            current = arc.iloc[0]["to"]
        if ordered:
            ordered_frames.append(pd.DataFrame(ordered))

    if not ordered_frames:
        return route_df.iloc[0:0].copy()

    return pd.concat(ordered_frames, ignore_index=True)


def _build_route_df(model) -> pd.DataFrame:
    """Build the ordered fleet route dataframe with a per-vehicle column."""
    route_records = [
        {
            "vehicle": k,
            "from": i,
            ...
            "load_leave": int(value(model.w[i, k])),
        }
        for (i, j) in model.ARCS
        for k in model.VEHICLES
    ...
    if route_df.empty:
        return route_df
    return _order_routes_by_sequence(route_df)
...
def _save_metrics(model, result, ctx: RunOptimizationContext) -> None:
    """Save optimization metrics to JSON (fleet-wide plus per-vehicle)."""
    ...
    # Per-vehicle metrics, then fleet aggregates
    per_vehicle = {k: _vehicle_metrics(model, k) for k in model.VEHICLES}
    used_vehicles = [k for k in model.VEHICLES if _vehicle_used(model, k)]

    travel_time = sum(m["travel_time"] for m in per_vehicle.values())
    station_time = sum(m["station_time"] for m in per_vehicle.values())
    depot_time = sum(m["depot_time"] for m in per_vehicle.values())
    total_time = travel_time + station_time + depot_time
    max_vehicle_time = max((per_vehicle[k]["total_time"] for k in used_vehicles), default=0.0)
    total_distance_m = sum(m["distance_m"] for m in per_vehicle.values())
    ...
    # Fleet-wide visited stations
    num_visited = sum(
        1
        for h in model.STATIONS
        if sum(
            value(model.x[i, j, k]) for (i, j) in model.ARCS if j == h for k in model.VEHICLES
        )
        > 0.5
    )

    total_bikes_moved = sum(_node_pickups(model, i) for i in model.NODES)

    # Deviation metrics (station-level, fleet-wide outcome)
    ...
    vehicle_breakdown = {
        int(k): {
            "used": k in used_vehicles,
            "distance_km": round(per_vehicle[k]["distance_m"] / 1000, 3),
            "total_time_min": round(per_vehicle[k]["total_time"], 1),
            "travel_time_min": round(per_vehicle[k]["travel_time"], 1),
            "station_time_min": round(per_vehicle[k]["station_time"], 1),
            "depot_time_min": round(per_vehicle[k]["depot_time"], 1),
            "num_stations_visited": per_vehicle[k]["num_stations"],
            "bikes_moved": per_vehicle[k]["bikes_moved"],
        }
        for k in model.VEHICLES
    }
    ...
    results_metrics = {
        ...
        "fleet_operation_time_min": round(total_time, 1),
        "max_vehicle_time_min": round(max_vehicle_time, 1),
        ...
        "num_vehicles_used": len(used_vehicles),
        "num_vehicles_available": len(model.VEHICLES),
        "per_vehicle": vehicle_breakdown,
    }
...
def _save_stations(model, ctx: RunOptimizationContext) -> None:
    """Save station-level results to CSV (operations aggregated over the fleet)."""
    station_vehicle = _station_vehicle_map(model)

    ops_records = []
    for i in model.NODES:
        pickups = _node_pickups(model, i)
        dropoffs = _node_dropoffs(model, i)
        ...
        if i in model.STATIONS:
            ...
            served_by = station_vehicle.get(i)
        else:
            # Depots have no b_final variable; derive final inventory from flows
            if i == DEPOT_START:
        ...
        visited = (
            sum(value(model.x[ia, j, k]) for (ia, j) in model.ARCS if ia == i for k in model.VEHICLES)
            > 0.5
        )
...
def _save_route(model, network_df: gpd.GeoDataFrame, ctx: RunOptimizationContext) -> None:
    """Save fleet route to CSV and shapefile, with a per-vehicle column."""
    route_df = _build_route_df(model)
    ...
    if route_df.empty:
        logger.info("No arcs traversed; skipping route shapefile")
        return
...
def _save_map(
    ...
    """Save the multi-vehicle rebalancing map visualization."""
    ...
    # Ordered per-vehicle route arcs traveled
    route_df = _build_route_df(model)

    # Prepare journey geodataframe (keeps the vehicle column for coloring)
    if route_df.empty:
        journey_gdf = gpd.GeoDataFrame(
            columns=["vehicle", "from", "to", "geometry"], geometry="geometry", crs="EPSG:4326"
        )
    else:
...
def run_optimization(ctx: RunOptimizationContext) -> None:
    """
    Run multi-vehicle bike rebalancing optimization for target date.

    Args:
        ctx: RunOptimizationContext containing configuration and output paths
    """
    logger.info("Starting fleet rebalancing optimization")
    logger.info(f"Target date: {ctx.target_date}")
    logger.info(f"Truck capacity: {ctx.model_params['truck_capacity']} bikes")
    logger.info(f"Fleet size: {ctx.model_params['fleet_size']} vehicles")
    logger.info(f"Depot capacity: {ctx.model_params['depot_capacity']} bikes")
